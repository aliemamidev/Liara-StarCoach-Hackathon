...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [chats, recentMessages, totalMessages] = await Promise.all([
      ...
      prisma.message.findMany({ where: { createdAt: { gte: since } }, select: { createdAt: true, role: true } }),
    ...
    return res.status(200).json({
      dashboard: {
        kpis: [
          ...
          { id: "failures", label: "درخواست ناموفق", value: "۰", change: "واقعی", note: "خطای ثبت‌شده در داده‌ها", tone: "red", trend: weekly.map(() => 0) },
        ],
        weekly,
      ...
      analytics: { weekly, totalMessages, unanswered: unanswered.length, activeToday },
    });
  } catch (error) {
