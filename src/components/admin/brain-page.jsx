...
export function BrainPage() {
  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-col gap-3 rounded-2xl border border-[hsl(var(--site-border))] bg-[hsl(var(--site-surface))] p-4 sm:flex-row sm:items-center">
      ...
      {error && <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-700" role="alert">{error}</div>}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent dir="rtl" className="max-w-2xl"><DialogHeader><DialogTitle>{form.id ? "ویرایش دانش" : "افزودن دانش"}</DialogTitle><DialogDescription>این پاسخ در جست‌وجوی بعدی Lia استفاده می‌شود.</DialogDescription></DialogHeader><form onSubmit={save} className="mt-4 space-y-4"><label className="block text-xs font-bold">سؤال<input value={form.question} onChange={(event) => setForm({ ...form, question: event.target.value })} className="mt-2 min-h-11 w-full rounded-xl border border-[hsl(var(--site-border))] bg-transparent px-3 text-sm" required /></label><label className="block text-xs font-bold">پاسخ<textarea value={form.answer} onChange={(event) => setForm({ ...form, answer: event.target.value })} className="mt-2 min-h-40 w-full rounded-xl border border-[hsl(var(--site-border))] bg-transparent p-3 text-sm leading-6" required /></label><label className="block text-xs font-bold">برچسب‌ها<textarea value={form.tags} onChange={(event) => setForm({ ...form, tags: event.target.value })} className="mt-2 min-h-11 w-full rounded-xl border border-[hsl(var(--site-border))] bg-transparent p-3 text-sm" placeholder="مثلاً: dns, خطا" /></label><div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setDialogOpen(false)}>انصراف</Button><Button type="submit" disabled={saving}>{saving && <LoaderCircle className="animate-spin" size={16} />}{saving ? "در حال ذخیره" : "ذخیره دانش"}<Check size={16} /></Button></div></form></DialogContent></Dialog>
    </section>
  );
}
