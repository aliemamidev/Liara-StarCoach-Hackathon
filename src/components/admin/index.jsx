...
function AdminSidebar({ activeSection, onNavigate, collapsed = false, onToggle, mobile = false, system = {}, lastUpdated = "در حال دریافت" }) {
  const connected = system.database === "connected";
  return (
    <div className={`flex h-full flex-col ${mobile ? "p-5" : collapsed ? "px-3 py-7" : "px-5 py-7"}`}>
      ...
      <div className="mt-10 flex min-h-0 flex-1 flex-col">
        ...
        <nav className="min-h-0 flex-1 space-y-1.5 overflow-y-auto overscroll-contain pb-4" aria-label="ناوبری پنل ادمین">
      ...
      {!collapsed && <div className="mt-4 shrink-0">
        <div className="rounded-2xl border border-[#2a3b56] bg-[#17263d] p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className={`text-[10px] font-bold ${connected ? "text-[#64e4d2]" : "text-[#f29a5c]"}`}>{connected ? "متصل" : "نامشخص"}</span>
          </div>
          ...
          <p className="mt-2 text-[10px] leading-5 text-[#7990ab]">آخرین همگام‌سازی: {lastUpdated}</p>
        </div>
      </div>}
    </div>
  );
...
function OverviewChart({ items = [] }) {
  ...
  return <Card className="p-5"><div className="mb-8 flex flex-wrap items-center justify-between gap-3"><div><p className="text-[11px] font-bold text-[#8b99ab]">روند تعاملات</p><h2 className="mt-1 text-[17px] font-extrabold text-[#1a2940]">حجم پیام‌ها در هفته جاری</h2></div><span className="flex items-center gap-2 rounded-xl border border-[#e7ecf2] px-3 py-2 text-[11px] font-bold text-[#65738a]"><CalendarDays size={14} />دادهٔ واقعی ۷ روز اخیر</span></div>{items.length === 0 ? <EmptyState title="هنوز داده‌ای برای نمودار نیست" description="با ثبت مکالمه، روند واقعی اینجا نمایش داده می‌شود." /> : <div className="flex h-48 items-end gap-2 sm:gap-4"><div className="flex h-full flex-col justify-between pb-7 text-[10px] text-[#b0bac8]"><span>۱۰۰</span><span>۷۵</span><span>۵۰</span><span>۲۵</span><span>۰</span></div><div className="relative flex h-full flex-1 items-end justify-between gap-2 border-b border-[#edf0f4] pb-7 before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-[#f1f3f6] before:shadow-[0_48px_0_#f1f3f6,0_96px_0_#f1f3f6,0_144px_0_#f1f3f6]">{items.map((item, index) => <div className="z-10 flex h-full flex-1 flex-col items-center justify-end gap-2" key={item.day}><div className={`w-full max-w-8 rounded-t-xl transition hover:opacity-80 ${index === items.length - 1 ? "bg-gradient-to-t from-[#3f72e1] to-[#70e6d4] shadow-[0_8px_18px_rgba(70,125,224,.22)]" : "bg-[#dfe8f7]"}`} style={{ height: `${(item.value / max) * 78}%` }} /><span className="absolute bottom-0 text-[10px] font-semibold text-[#9aa6b7]">{item.day}</span></div>)}</div></div>}</Card>;
}

function TopicsCard({ onNavigate, items = [] }) {
  return <Card className="p-5"><SectionTitle label="موضوعات پرتکرار" action="مشاهده تحلیل کامل" onAction={() => onNavigate("analytics")} />{items.length === 0 ? <EmptyState title="موضوع پرتکراری ثبت نشده" description="پس از ثبت پیام‌های واقعی، این بخش از دیتابیس پر می‌شود." /> : <div className="space-y-5">{items.map((topic) => <div key={topic.label}><div className="mb-2 flex items-center justify-between text-[11px]"><span className="font-bold text-[#4f5f74]">{topic.label}</span><span className="font-extrabold text-[#1e2d43]">{topic.count}</span></div><div className="h-2 overflow-hidden rounded-full bg-[#eef2f6]"><div className={`h-full rounded-full ${toneClasses[topic.tone]?.line || toneClasses.blue.line}`} style={{ width: `${topic.percentage}%` }} /></div></div>)}</div>}</Card>;
}

function SystemPulse({ system = {} }) {
  const connected = system.database === "connected";
  const bars = [18, 25, 20, 36, 28, 46, 34, 40, 27, 51, 43, 58];
  return <Card className="overflow-hidden border-0 bg-[#14233a] p-0 text-white"><div className="relative p-5"><div className="absolute -left-8 -top-10 h-36 w-36 rounded-full bg-[#64e4d2]/10 blur-2xl" /><div className="relative flex items-start justify-between"><div><p className="text-[11px] font-bold text-[#91a8c4]">وضعیت سیستم</p><h2 className="mt-1 text-[18px] font-extrabold">{connected ? "دیتابیس متصل است" : "اتصال دیتابیس برقرار نیست"}</h2></div><span className={`flex h-9 w-9 items-center justify-center rounded-xl ${connected ? "bg-[#64e4d2]/15 text-[#64e4d2]" : "bg-[#f29a5c]/15 text-[#f29a5c]"}`}><ShieldCheck size={19} /></span></div><div className="relative mt-6 flex items-end gap-1.5">{bars.map((height, index) => <span key={index} className={`flex-1 rounded-full ${connected ? "bg-[#64e4d2]" : "bg-[#f29a5c]"}`} style={{ height }} />)}</div><div className="relative mt-4 flex items-center justify-between border-t border-white/10 pt-3 text-[10px]"><span className="text-[#8ca1bb]">دادهٔ عملیاتی</span><span className={`font-bold ${connected ? "text-[#64e4d2]" : "text-[#f29a5c]"}`}>{connected ? "زنده" : "نامشخص"}</span></div></div><div className="grid grid-cols-3 border-t border-white/10 bg-white/[.03] px-5 py-4 text-center"><div><p className="text-[15px] font-extrabold">{system.chatCount ?? 0}</p><p className="mt-1 text-[9px] text-[#8499b4]">چت ثبت‌شده</p></div><div className="border-x border-white/10"><p className="text-[15px] font-extrabold">{system.pendingEscalations ?? 0}</p><p className="mt-1 text-[9px] text-[#8499b4]">ارجاع باز</p></div><div><p className="text-[15px] font-extrabold">{system.activeToday ?? 0}</p><p className="mt-1 text-[9px] text-[#8499b4]">فعال امروز</p></div></div></Card>;
}
...
function ConversationDialog({ message, onClose }) {
  ...
  return <Dialog open={Boolean(message)} onOpenChange={(open) => !open && onClose()}><DialogContent className="max-h-[calc(100dvh-1rem)] w-[min(560px,calc(100vw-1rem))] overflow-y-auto border-[#e4eaf1] bg-white p-0 text-right shadow-[0_24px_80px_rgba(27,47,76,.2)]"><DialogHeader className="border-b border-[#edf0f4] bg-[#fbfcfe] p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-bold tracking-[0.12em] text-[#8b99ab]">جزئیات مکالمه</p><DialogTitle className="mt-1 truncate text-base font-extrabold text-[#1a2940]">{message?.user || "مکالمه"}</DialogTitle><DialogDescription className="mt-1 text-[11px] text-[#8390a2]">{message?.time} · {message?.topic}</DialogDescription></div>{message && <StatusBadge status={message.status}>{message.statusLabel}</StatusBadge>}</div></DialogHeader>{message && <div className="space-y-3 p-4">{messages.length > 0 ? messages.map((item, index) => <div key={`${item.id || item.createdAt || "message"}-${index}`} className={`rounded-xl p-3 ${item.role === "assistant" ? "border border-[#dcefe9] bg-[#f4fcfa]" : "bg-[#f5f7fb]"}`}><div className={`mb-1 flex items-center gap-2 text-[10px] font-bold ${item.role === "assistant" ? "text-[#159787]" : "text-[#8997a8]"}`}>{item.role === "assistant" ? <Bot size={15} /> : <UserRound size={15} />}{item.role === "assistant" ? "پاسخ دستیار" : "پیام کاربر"}</div><p className={`whitespace-pre-wrap break-words text-[13px] leading-6 ${item.role === "assistant" ? "text-[#41665f]" : "font-semibold text-[#34445b]"}`}>{item.text || item.content || ""}</p></div>) : <><div className="rounded-xl bg-[#f5f7fb] p-3"><div className="mb-1 flex items-center gap-2 text-[10px] font-bold text-[#8997a8]"><UserRound size={15} />پیام کاربر</div><p className="whitespace-pre-wrap break-words text-[13px] font-semibold leading-6 text-[#34445b]">{message.question}</p></div><div className="rounded-xl border border-[#dcefe9] bg-[#f4fcfa] p-3"><div className="mb-1 flex items-center gap-2 text-[10px] font-bold text-[#159787]"><Bot size={15} />پاسخ دستیار</div><p className="whitespace-pre-wrap break-words text-[13px] leading-6 text-[#41665f]">{message.response}</p></div></>}<div className="flex flex-wrap items-center justify-between gap-3 border-t border-[#edf0f4] pt-3"><div className="flex items-center gap-2 text-[10px] text-[#8b99ab]"><Database size={15} />منبع: پایگاه دادهٔ عملیاتی</div><Button variant="outline" size="sm" className="min-h-11 border-[#e3eaf1] text-[10px]" onClick={onClose}><X size={15} />بستن</Button></div></div>}</DialogContent></Dialog>;
}
...
function AdminDashboardShell({ session }) {
  ...
  const loadOverview = () => {
    ...
    return fetch("/api/admin/overview", { credentials: "same-origin", cache: "no-store" }).then((response) => response.ok ? response.json() : Promise.reject(new Error("overview unavailable"))).then((data) => {
      ...
      setLoading(false);
      setLoadError("دریافت اطلاعات واقعی پنل ممکن نشد. اتصال دیتابیس یا نشست ادمین را بررسی کنید.");
      throw error;
    });
  };
  ...
  const openNotification = async (notification) => {
    ...
    setSelectedMessage({ user: "در حال دریافت…", topic: "دادهٔ واقعی", status: "review", statusLabel: "در حال بارگذاری", time: notification.time, question: "", response: "", messages: [] });
    try {
      ...
      setSelectedMessage({ id: item.id, user: item.user?.name || item.email || "کاربر مهمان", topic: "گفتگوی واقعی", status: lastAssistant ? "answered" : "review", statusLabel: lastAssistant ? "پاسخ داده شده" : "نیازمند بررسی", time: item.updatedAt ? new Date(item.updatedAt).toLocaleString("fa-IR") : notification.time, question: lastUser?.text || item.title, response: lastAssistant?.text || "پاسخی برای این مکالمه ثبت نشده است.", messages });
    } catch (error) {
