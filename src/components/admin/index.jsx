...
function OverviewChart({ items = [] }) {
  ...
  return <Card className="p-5"><div className="mb-6 flex flex-wrap items-center justify-between gap-3"><div><p className="text-[11px] font-bold text-[#8b99ab]">روند تعاملات</p><h2 className="mt-1 text-[17px] font-extrabold text-[#1a2940]">حجم پیام‌ها در هفته جاری</h2></div><span className="flex items-center gap-2 rounded-xl border border-[#e7ecf2] px-3 py-2 text-[11px] font-bold text-[#65738a]"><CalendarDays size={14} />روند ۷ روز اخیر</span></div>{items.length === 0 ? <EmptyState title="هنوز داده‌ای برای نمودار نیست" /> : <div role="img" aria-label="نمودار خطی حجم پیام‌های هفته جاری" dir="ltr"><svg viewBox={`0 0 ${width} ${height}`} className="h-56 w-full overflow-visible" preserveAspectRatio="none"><line x1={pad} x2={width - pad} y1={height - pad} y2={height - pad} stroke="hsl(var(--site-border))" /><polygon points={area} fill="rgba(78,121,232,.12)" /><polyline points={points} fill="none" stroke="#4e79e8" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />{items.map((item, index) => { const x = pad + (index * (width - pad * 2)) / Math.max(items.length - 1, 1); const y = height - pad - ((item.value / max) * (height - pad * 2)); return <circle key={`${item.day}-${index}`} cx={x} cy={y} r="4" fill="#fff" stroke="#4e79e8" strokeWidth="3"><title>{`${item.day}: ${item.value}`}</title></circle>; })}</svg><div className="grid grid-cols-7 gap-1 text-center text-[10px] font-semibold text-[#9aa6b7]">{items.map((item) => <span key={item.day} dir="rtl">{item.day}</span>)}</div></div>}</Card>;
}
...
function ToggleRow({ icon: RowIcon, title, description, checked, onChange }) {
  return <div className="admin-settings-toggle-row flex items-center gap-4 border-b border-[hsl(var(--site-border))] py-4 last:border-0"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[hsl(var(--site-accent)/.12)] text-[hsl(var(--site-accent-strong))]"><RowIcon size={18} /></span><div className="min-w-0 flex-1"><p className="text-[12px] font-extrabold text-[hsl(var(--site-text))]">{title}</p><p className="mt-1 text-[10px] leading-5 text-[hsl(var(--site-muted))]">{description}</p></div><button type="button" role="switch" aria-checked={checked} aria-label={`${title}: ${checked ? "روشن" : "خاموش"}`} onClick={() => onChange(!checked)} className={`admin-toggle admin-settings-toggle-control relative flex h-7 w-12 shrink-0 items-center rounded-full transition ${checked ? "is-on" : "is-off"}`}><span className="absolute top-1 h-5 w-5 rounded-full bg-white shadow-[0_1px_3px_rgba(26,43,67,.28)] transition" /></button></div>;
}
...
function ThemeControls() {
  ...
  useEffect(() => setMounted(true), []);
  const isDark = mounted && resolvedTheme === "dark";
  return <div className="admin-auth-theme-controls fixed start-4 top-4 z-20 flex items-center gap-2"><button type="button" onClick={() => { setTheme(isDark ? "light" : "dark"); sound.playSound("toggle"); }} aria-label={isDark ? "فعال‌کردن حالت روشن" : "فعال‌کردن حالت تاریک"} aria-pressed={isDark} className="flex h-11 w-11 items-center justify-center rounded-xl border border-[hsl(var(--site-border))] bg-[hsl(var(--site-surface)/.8)] text-[hsl(var(--site-muted))] shadow-sm backdrop-blur transition hover:text-[hsl(var(--site-accent-strong))] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--site-accent))]">{isDark ? <Sun size={18} /> : <Moon size={18} />}</button><button type="button" onClick={() => sound.setEnabled(!sound.enabled)} aria-label={sound.enabled ? "خاموش‌کردن افکت صوتی" : "روشن‌کردن افکت صوتی"} aria-pressed={sound.enabled} className="flex h-11 w-11 items-center justify-center rounded-xl border border-[hsl(var(--site-border))] bg-[hsl(var(--site-surface)/.8)] text-[hsl(var(--site-muted))] shadow-sm backdrop-blur transition hover:text-[hsl(var(--site-accent-strong))] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--site-accent))]">{sound.enabled ? <Volume2 size={18} /> : <VolumeX size={18} />}</button></div>;
}

function AdminAuthView() {
  return <><Head><title>ورود به پنل مدیریت | لیارا</title><meta name="description" content="ورود امن به پنل مدیریت لیارا" /></Head><main dir="rtl" className="admin-auth-main admin-auth-shell relative flex min-h-dvh max-h-none items-start justify-center overflow-y-auto bg-[hsl(var(--site-bg))] px-4 py-4 sm:px-6 lg:items-center lg:py-8"><ThemeControls /><div className="relative h-auto w-full max-w-[1120px]"><LoginForm /></div></main></>;
}
